var constant={NODE_STRING:["<EMPTY>","<NULL>"],codeMirrorPath:{air:"core/extern/CodeMirror-0.7",web:"web/core/extern/CodeMirror-0.7"}};function isObject(what){return typeof what=="object"}function isArray(obj){if(obj.constructor.toString().indexOf("Array")==-1)return false;else return true}function isUrl(s){var regexp=/(ftp|http|https):\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/;return regexp.test(s)}function getObjectType(obj){var type="";if(isObject(obj))type="object";if(isArray(obj))type="array";return type}function isString(a){return typeof a=="string"}
function openUrl(url){debug.trace("--OPEN URL '"+url+"'--");var urlReq=new air.URLRequest(url);air.navigateToURL(urlReq)}
...
Array.prototype.in_array=function(needle){for(var i=0;i<this.length;i++)if(this[i]===needle)return true;return false};var JPAir={init:function(){window.nativeWindow.addEventListener(air.Event.CLOSING,JPAir.events.onClosingWindow);air.NativeApplication.nativeApplication.addEventListener(air.Event.EXITING,JPAir.events.exitingApplication);air.NativeApplication.nativeApplication.addEventListener(air.InvokeEvent.INVOKE,JPAir.events.invokeApplication);JPAir.updater.init();JPAir.preferences.init()}};
JPAir.app={xmlInfo:"",init:function(){},getApplicationInfo:function(){debug.trace("xx"+JPAir.app.xmlInfo);var appXml=new DOMParser;var xmlObject=appXml.parseFromString(JPAir.app.xmlInfo,"text/xml");var root=xmlObject.getElementsByTagName("application")[0];var initialWindow=root.getElementsByTagName("initialWindow")[0];return[root.getElementsByTagName("version")[0].firstChild.data,root.getElementsByTagName("name")[0].firstChild.data,initialWindow.getElementsByTagName("content")[0].firstChild.data]},
getApplicationVersion:function(){return this.getApplicationInfo()[0]},getApplicationName:function(){return this.getApplicationInfo()[1]}};
JPAir.events={onClosingWindow:function(event){JPAir.preferences.saveData({syntax_hl:Ext.getCmp("JPviewPort").findByType("jp_main_top")[0].getTopToolbar().findByType("jp_main_top_iconbar")[0].btnGroup_others.switchHighlighting.pressed});event.preventDefault();nativeWindow.removeEventListener("closing",JPAir.events.onClosingWindow);JP.util.air.exit()},exitingApplication:function(event){var opened=air.NativeApplication.nativeApplication.openedWindows;for(var i=0;i<opened.length;i++)opened[i].close()},
invokeApplication:function(event){if(event.arguments.length>0){var path=event.arguments[0];var file=new air.File(path);var stream=new air.FileStream;stream.open(file,air.FileMode.READ);var jsonString=stream.readUTFBytes(stream.bytesAvailable);stream.close();Ext.getCmp("JPviewPort").findByType("jp_main_center_stringForm")[0].findByType("ux-codemirror")[0].setValue(jsonString)}},onSaveAsSelect:function(event){event.target.removeEventListener(air.Event.SELECT,JPAir.events.onSaveAsSelect);var currentFile=
event.target;var txt=Ext.getCmp("JPviewPort").findByType("jp_main_center_stringForm")[0].findByType("ux-codemirror")[0].getValue();try{var stream=new air.FileStream;stream.open(currentFile,air.FileMode.WRITE);stream.writeUTFBytes(txt);stream.close()}catch(error){alert("Error reading or writing the file.")}}};
JPAir.updater={appUpdater:null,configXml:"app:/update-config.xml",init:function(){JPAir.updater.appUpdater=new runtime.air.update.ApplicationUpdaterUI;JPAir.updater.appUpdater.configurationFile=new air.File(JPAir.updater.configXml);JPAir.updater.appUpdater.addEventListener(air.ErrorEvent.ERROR,JPAir.updater.onError);JPAir.updater.appUpdater.addEventListener("initialized",JPAir.updater.onInitialization);JPAir.updater.appUpdater.initialize()},checkUpdate:function(){JPAir.updater.appUpdater.checkNow()},
onInitialization:function(){JPAir.updater.checkUpdate()},onError:function(event){alert(event.toString())}};
JPAir.preferences={prefsFile:null,prefsXML:null,stream:null,settings:null,init:function(){this.prefsFile=air.File.applicationStorageDirectory;this.prefsFile=this.prefsFile.resolvePath("settings.xml");this.readXML()},readXML:function(){this.stream=new air.FileStream;if(this.prefsFile.exists){this.stream.open(this.prefsFile,air.FileMode.READ);this.processXMLData()}else this.saveData(null);window.nativeWindow.visible=true},processXMLData:function(){this.prefsXML=this.stream.readUTFBytes(this.stream.bytesAvailable);
this.stream.close();var domParser=new DOMParser;this.prefsXML=domParser.parseFromString(this.prefsXML,"text/xml");var syntaxHl=this.prefsXML.getElementsByTagName("syntaxHighlighting")[0];this.settings={syntax_hl:syntaxHl.getAttribute("showCodeMirror")}},saveData:function(data){this.createXMLData(data);this.writeXMLData()},createXMLData:function(data){if(data==null)data={syntax_hl:true};var cr=air.File.lineEnding;this.prefsXML="<?xml version='1.0' encoding='utf-8'?>"+cr+"<preferences>"+cr+"    <syntaxHighlighting showCodeMirror='"+
(data.syntax_hl?"true":"false")+"' />"+cr+"    <saveDate>"+(new Date).toString()+"</saveDate>"+cr+"</preferences>"},writeXMLData:function(){this.stream=new air.FileStream;this.stream.open(this.prefsFile,air.FileMode.WRITE);this.stream.writeUTFBytes(this.prefsXML);this.stream.close()}};var JSLINT=function(){var adsafe_id,adsafe_may,adsafe_went,anonname,approved,atrule={media:true,"font-face":true,page:true},bang={"<":true,"<=":true,"==":true,"===":true,"!==":true,"!=":true,">":true,">=":true,"+":true,"-":true,"*":true,"/":true,"%":true},banned={arguments:true,callee:true,caller:true,constructor:true,eval:true,prototype:true,stack:true,unwatch:true,valueOf:true,watch:true},boolOptions={adsafe:true,bitwise:true,browser:true,cap:true,css:true,debug:true,devel:true,eqeqeq:true,es5:true,
...
*/
if(Ext.isAir)console={log:air.Introspector?air.Introspector.Console.log:function(){}};function init(){if(Ext.isAir){JPAir.app.xmlInfo=air.NativeApplication.nativeApplication.applicationDescriptor;JPAir.init()}else $.get("application.xml",function(data){JPAir.app.xmlInfo=data});if(JPAir.app.getApplicationInfo()[2]=="JSONpad-air.html")air.Introspector.Console.info()}Ext.BLANK_IMAGE_URL="core/extjs/resources/images/default/s.gif";Ext.ns("JP");
Ext.onReady(function(){Ext.QuickTips.init();var viewport=new Ext.Viewport({layout:"border",id:"JPviewPort",items:[{xtype:"jp_main_top"},{xtype:"jp_main_center"},{xtype:"jp_main_bottom"}]})});JP.util={exit:function(){if(Ext.isAir)JP.util.air.exit();else JP.util.web.exit()},copyToClipboard:function(string){if(Ext.isAir)JP.util.air.copyToClipboard(string);else JP.util.web.copyToClipboard(string)},pasteFromClipboard:function(){if(Ext.isAir)return JP.util.air.pasteFromClipboard();else return JP.util.web.pasteFromClipboard()},validateJson:function(json,clear){if(!json.errorObject){if(clear)JP.util.clearCodeMirrorStatus();return true}else{JP.util.setCodeMirrorStatus({text:json.errorObject.length+
" error/s in JSON string. Click for details...",iconCls:"x-status-error x-status-error-detail-link",clear:false});if(!JP.ErrorWindow.Action.window)JP.ErrorWindow.Action.init();JP.ErrorWindow.Action.window.setTitle(json.errorObject.length+" error/s found");var msg="";for(var i=0;i<json.errorObject.length;i++){msg+='<li style="list-style-type:disc; margin-left: 18px;">'+json.errorObject[i].msg+"</li>";if(json.errorObject[i].evi)msg+='<li style="list-style-type:none; padding-bottom: 2px; margin-left: 6px; font-family: monospace;">-&gt; '+
...
null)JP.XmlWindow.Action.me=this;JP.XmlWindow.Action.openWindow()}};JP.MainTop.Action.mainBar={saveAsFile:function(){if(Ext.isAir){var fileChooser=air.File.documentsDirectory;fileChooser.browseForSave("Save");fileChooser.addEventListener(air.Event.SELECT,JPAir.events.onSaveAsSelect)}},quitApplication:function(){JP.util.exit()},openAboutWindow:function(){if(JP.AboutWindow.Action.me==null)JP.AboutWindow.Action.me=this;JP.AboutWindow.Action.openWindow()}};
...
false))JP.util.setCodeMirrorStatus({text:"JSON string is valid",iconCls:"x-status-valid",clear:true})}},switchHighlighting:function(btn,pressed){var stringInput=this.findParentByType("viewport").findByType("jp_main_center_stringForm")[0].findByType("ux-codemirror")[0];if(pressed==true)stringInput.showCodeMirror();else stringInput.hideCodeMirror()},insertExample:function(){this.findParentByType("viewport").findByType("jp_main_center_stringForm")[0].findByType("ux-codemirror")[0].setValue(this.example)},
fetchJsonByUrl:function(field,value){var me=this;if(!isUrl(value))Ext.Msg.show({title:"No valid url",msg:"Please enter a valid url with http://!",buttons:Ext.MessageBox.OK,icon:Ext.MessageBox.ERROR});else{var conn=new Ext.data.Connection;conn.request({url:value,method:"POST",success:function(responseObject){var stringInputField=me.findParentByType("viewport").findByType("jp_main_center_stringForm")[0].findByType("ux-codemirror")[0];var json=JP.util.parseJson(responseObject.responseText,true);if(JP.util.validateJson(json,
true))JP.util.setJPStatus({text:"Fetched JSON string succesfully",iconCls:"x-status-valid",clear:true},"left");stringInputField.setValue(JSON.stringify(json,null,"  "))},failure:function(){Ext.Msg.show({title:"Fetching error",msg:"Unable to fetch JSON string from url: "+value,buttons:Ext.MessageBox.OK,icon:Ext.MessageBox.ERROR})}})}}};JP.MainTop.mainBar=Ext.extend(Ext.Toolbar,{initComponent:function(){this.items=[{text:"File",ref:"file",menu:{ref:"menu",items:[{text:"Save as file..",statusBarTip:"Save the JSON string as a file",ref:"save_file"},{text:"Quit",statusBarTip:"Quit the application",ref:"quit"}]}},{text:"Edit",ref:"edit",menu:{ref:"menu",items:[{text:"XML2JSON Converter",ref:"xml2json",statusBarTip:"Convert XML data to a JSON string and insert it in the textarea"},"-",{text:"Copy JSON string",ref:"copy",statusBarTip:"Copy JSON string to clipboard"},
{text:"Paste JSON string",ref:"paste",statusBarTip:"Paste JSON string from cliboard"}]}},{text:"Help",ref:"help",menu:{ref:"menu",items:[{text:"Check for updates",ref:"checkUpdate",statusBarTip:"Check if updates available"},"-",{text:"About...",ref:"about",statusBarTip:"About the application"}]}}];JP.MainTop.mainBar.superclass.initComponent.call(this);this.file.menu.quit.setHandler(JP.MainTop.Action.mainBar.quitApplication);this.file.menu.save_file.setHandler(JP.MainTop.Action.mainBar.saveAsFile);
this.edit.menu.xml2json.setHandler(JP.MainTop.Action.openXmlWindow,this);this.edit.menu.copy.setHandler(JP.MainTop.Action.copyJsonStringToClipboard,this);this.edit.menu.paste.setHandler(JP.MainTop.Action.pasteJsonStringFromClipboard,this);this.help.menu.checkUpdate.setHandler(function(){JPAir.updater.checkUpdate(true)},this);this.help.menu.about.setHandler(JP.MainTop.Action.mainBar.openAboutWindow,this);this.file.menu.addListener("mouseover",this.onMenuMouseover);this.file.menu.addListener("mouseout",
this.onMenuMouseout);this.edit.menu.addListener("mouseover",this.onMenuMouseover);this.edit.menu.addListener("mouseout",this.onMenuMouseout);this.help.menu.addListener("mouseover",this.onMenuMouseover);this.help.menu.addListener("mouseout",this.onMenuMouseout)},onMenuMouseover:function(menu,e,menuItem){JP.util.setJPStatus({text:menuItem.statusBarTip,clear:false},"left")},onMenuMouseout:function(){JP.util.setJPStatus({text:"",clear:false},"left")}});Ext.reg("jp_main_top_mainbar",JP.MainTop.mainBar);JP.MainTop.iconBar=Ext.extend(Ext.Toolbar,{initComponent:function(){this.items=[{xtype:"buttongroup",title:"Tree",columns:2,ref:"btnGroup_tree",items:[{iconCls:"icon_loadToTree",scale:"medium",tooltip:"Load JSON String to tree",ref:"toTree"},{iconCls:"icon_loadFromTree",tooltip:"Load data from tree as JSON string",scale:"medium",ref:"fromTree"}]},{xtype:"buttongroup",title:"Format",columns:2,ref:"btnGroup_format",items:[{iconCls:"icon_format",scale:"medium",tooltip:"Format JSON string",ref:"indent"},
{iconCls:"icon_compress",scale:"medium",tooltip:"Compress JSON string",ref:"compress"}]},{xtype:"buttongroup",title:"Clipboard",columns:2,ref:"btnGroup_clipboard",items:[{iconCls:"icon_copy",scale:"medium",tooltip:"Copy JSON into clipboard",ref:"copy"},{iconCls:"icon_paste",scale:"medium",tooltip:"Paste JSON from clipboard",ref:"paste"}]},{xtype:"buttongroup",title:"Tools",columns:4,ref:"btnGroup_others",items:[{iconCls:"icon_loadFromUrl",scale:"medium",tooltip:"Load JSON string by an url",ref:"loadFromUrl",
menu:{ref:"menu",items:[new Ext.ux.form.FetchField({emptyText:"Enter an url...",width:320,ref:"fetchfield"})]}},{iconCls:"icon_convertXml",scale:"medium",tooltip:"Convert XML data to JSON",ref:"convertXML"},{iconCls:"icon_validate",scale:"medium",tooltip:"Validate JSON string",ref:"validateJson"},{iconCls:"icon_highlight",id:"BtnJsonStringSyntaxHighlighting",scale:"medium",tooltip:"Turn on/off syntax highlighting",ref:"switchHighlighting",enableToggle:true,pressed:true}]},"->",{text:"Examples",scale:"large",
tooltip:"Insert a JSON example",arrowAlign:"left",ref:"btnExamples",menu:{ref:"menu",items:[{text:"Example 1",ref:"example1",example:'{\n\t"Key": "value"\n}'},{text:"Example 2",ref:"example2",example:'{\n\t"CreditCard" : "MasterCard",\n\t"Number" : "1234-5678-9012-3456",\n\t"Holder" : {\n\t\t"Name" : "Rich",\n\t\t"firstName" : "Rainer",\n\t\t"sex" : "male",\n\t\t"Likes" : [\n\t\t\t"Riding",\n\t\t\t"Swimming",\n\t\t\t"Reading"\n\t\t],\n\t\t"Age" : null\n\t},\n\t"Covering" : 2000000,\n\t"Currency" : "EURO"\n}'},
...
this);this.btnGroup_others.convertXML.setHandler(JP.MainTop.Action.openXmlWindow,this);this.btnGroup_others.loadFromUrl.menu.fetchfield.on("fetch",JP.MainTop.Action.iconBar.fetchJsonByUrl,this);this.btnGroup_others.validateJson.setHandler(JP.MainTop.Action.iconBar.validateJsonString,this);this.btnGroup_others.switchHighlighting.on("toggle",JP.MainTop.Action.iconBar.switchHighlighting,this);this.btnExamples.menu.example1.setHandler(JP.MainTop.Action.iconBar.insertExample,this.btnExamples.menu.example1);
this.btnExamples.menu.example2.setHandler(JP.MainTop.Action.iconBar.insertExample,this.btnExamples.menu.example2);this.btnExamples.menu.example3.setHandler(JP.MainTop.Action.iconBar.insertExample,this.btnExamples.menu.example3)}});Ext.reg("jp_main_top_iconbar",JP.MainTop.iconBar);JP.MainCenter=Ext.extend(Ext.Panel,{title:"",region:"center",layout:"border",border:false,initComponent:function(){this.items=[{xtype:"jp_main_center_stringForm"},{xtype:"jp_main_center_jsonTree"},{xtype:"jp_main_center_ediTreeForm"}];JP.MainCenter.superclass.initComponent.call(this)}});Ext.reg("jp_main_center",JP.MainCenter);JP.MainCenter.Action={};
...
scale:"medium",tooltip:"Turn on/off syntax highlighting",ref:"switchHighlighting",enableToggle:true,pressed:true}]}]};this.items=[{xtype:"ux-codemirror",codeMirrorPath:Ext.isAir?constant.codeMirrorPath.air:constant.codeMirrorPath.web,language:"xml",hideLabel:true,anchor:"100% 100%",name:"xmlData",ref:"xmlInput",onResizeCallBackFn:function(codeMirror){var el=Ext.select("."+codeMirror.id,true);var lineNumbersEl=Ext.select("."+codeMirror.id+" ~ div",true);var win=me;var width=win.getWidth();var height=
win.getHeight();if(el){if(codeMirror.initialized&&!codeMirror.codeMirrorHidden&&lineNumbersEl&&count(lineNumbersEl.elements)&&width)width=width-lineNumbersEl.elements[0].getWidth();for(var i=0;i<el.elements.length;i++){if(width&&!height)el.elements[i].setWidth(width);else if(!width&&height)el.elements[i].setHeight(height);else if(width&&height)el.elements[i].setSize(width-3,height-1);lineNumbersEl.elements[i].setHeight(height)}}},initCallBackFn:function(me){me.findParentByType("jp_xmlwindow").setWidth(me.findParentByType("jp_xmlwindow").getWidth()-
